...
- [x] Facilitator Workflow Improvements (32-player session sprint) — a bundle of workflow fixes gathered from real facilitator observations running a 32-player Open Play session:
  ...
  - ✅ Partner Requests — organizers can request that two players stay teamed together for as long as that request stands, at any point during the session (set at check-in time or mid-session), no separate session-wide setting to turn on first. Designed as a reusable matchmaking OPTION rather than a special-cased rotation mode or engine — implemented once, in `BalancedRotationEngine.buildTeams` (new `extractFixedPartnerTeams` helper, called unconditionally), the shared team-forming step every mode that composes `BalancedRotationEngine` already reuses. Concretely wired through and verified for **Continuous** (direct) and **Adaptive Skill Rotation** (via its `divisionEngine` composition — a requested pair is only ever forced together WITHIN one division's pool, since divisions are processed as separate calls, so Beginner/Intermediate separation is structurally untouched); Winner Pool Rotation's dedicated 4-player pooling algorithm and Progressive Skill Rotation's Transition/Competitive phases have their own separate scoring models that don't share this step and are unaffected either way, same as they already don't share other `BalancedRotationEngine`-specific behavior. A requested pair still goes through the exact same opponent-selection scoring (`scoreOpponents`) as any other team afterward, so opponent diversity keeps working; for Adaptive Skill Rotation, the resulting matchup still goes through the exact same games-played fairness tuple and Winner-vs-Winner bonus as any other candidate — only WHICH two players end up on the same team changed, never how matchups are scored or ranked afterward. New mutual `partnerId` field (session-scoped roster data, `null` default) set via new pure `setFixedPartner(state, idA, idB)`/`clearFixedPartner(state, playerId)` (`lib/queueManagement.js`) — reassigning a partner cleanly clears the old link on both sides, so `partnerId` is always genuinely mutual or null, never a stale one-sided pointer. Compact partner picker in the Waiting Players panel, always shown, per-pair — toggling a specific pair's request off (clearing it) is how an organizer stops pairing that pair up; everyone else keeps rotating exactly as before. A player with no requested partner is completely unaffected. 21 regression checks (`scripts/verify-permanent-partner-mode.mjs`).
...
- [x] Facilitator Workflow Improvements — Sprint 2 (real Open Play session observations). Adaptive Skill Rotation, the games-played fairness algorithm, Winner-vs-Winner, Promotion/Relegation, Smart Queue Management, and Smart Court Dispatch were explicitly out of scope this sprint and remain completely untouched.
  - ✅ Better Player Substitution — every candidate in the Substitute picker (live-court `CourtCard` or next-matchup `NextMatchupCard`) now shows Skill (BEG/INT), Games Played, and a live Waiting Time (`WaitingTimer`, same "since" fallback as everywhere else) alongside their name, plus a "⭐⭐ Recommended" tag with a specific reason for up to 3 flagged candidates: **Tournament partner** (the outgoing player's requested partner, if currently eligible to sub in — highest priority), **Longest waiting** (fills remaining slots, same ranking Substitute Recommendation already used), and **Same skill level** as the outgoing player (fills any slots still left). New pure `getSubstituteRecommendations(waitingPlayers, outgoingPlayer, count = 3)` (`lib/utils.js`) replaces the old longest-waiting-only `getRecommendedSubstitutes` for this purpose (that function itself is untouched — still used nowhere else) and returns `{id, reason}` pairs, each candidate getting exactly one reason (its highest-priority match) rather than being double-tagged. `PlayerPicker.jsx` now computes recommendations itself, locally, from a new `outgoingPlayer` prop (the full player record of whoever's being subbed out) — both `CourtCard`/`NextMatchupCard` already have that record on hand (`players[subbingId]`) the moment the picker renders, so the recommendation can never disagree with who's actually offered. A held player is never recommended, regardless of reason. Purely informational — never touches `queueIds`/`nextMatchups`/matchmaking, and the facilitator can still pick anyone else from the full list.
  - ✅ Remove Held Player Side Notifications — the floating, dismissible Held Player Reminder banner (`HeldPlayerReminderBanner.jsx`, deleted) is gone per direct facilitator feedback that it competed for attention during a busy session. Everything it was a thin UI layer over is completely unchanged: `holdPlayer`/`resumePlayer`, `heldAt`/`heldAtRound`/`heldReminderLastShownAt`, the configurable minutes/rounds/repeat-interval thresholds (still editable in Session Settings), and — critically — the underlying tick-based safeguard itself (`getPlayersNeedingHeldReminder`/`markHeldReminderShown`, still running every 15s once Scorer is unlocked) all still fire exactly as before; a reminder still gets recorded as a "Held Player Reminder" entry in the same shared Queue Activity Log every other Held Player action already writes to — a facilitator who wants to see it can still expand the log, it just no longer pops up unprompted.
  - ✅ Session Matchmaking Priority — a new Session Setting (`matchmakingPriority`: `null`/"Longest Waiting Time"/"Newly Checked-In"/"Least Games Played", `MATCHMAKING_PRIORITIES` in `lib/constants.js`), explicitly **not** a rotation mode: a reusable candidate-ORDERING policy that works alongside whichever Rotation Mode is active (Continuous + Least Games Played, Winner Pool + Newly Checked-In, Adaptive Skill Rotation + Longest Waiting, any combination). Implemented once, in the single choke point every rotation mode's freshly-generated matchups already flow through (`sortMatchupsByPriority`, called from `refreshNextMatchups`/`regenerateNextMatchups`, `lib/utils.js`) — zero engine-specific code, nothing added to any engine file, so this is never special-cased for Adaptive Skill Rotation or any other mode. Deliberately a POST-hoc re-sort of matchups an engine has ALREADY fully decided (who's on which team, who's facing whom): partner diversity, opponent diversity, Winner-vs-Winner, promotion/relegation, and Beginner/Intermediate division separation are all completely untouched, since none of that scoring is touched here — this only changes which of the already-built matchups sits at the FRONT of the list, which determines who gets dispatched first and who survives Smart Queue Management's queue-depth cap whenever there isn't room for everyone. `null` (the default — no priority explicitly set) returns matchups in the exact order the engine itself produced, so every existing session keeps its current tie-break behavior completely unchanged unless an organizer opts in via the new Session Settings dropdown.
  - ✅ Persistent Tournament Partners — extends Partner Requests (see below) with the explicit guarantee that a requested pair remains linked for the rest of the session — through Hold/Resume, Checkout, a skill change, or an automatic Promotion/Relegation — until the facilitator explicitly removes it, resuming automatically the instant both players are eligible again. This is largely already true of the existing implementation (nothing in `holdPlayer`/`resumePlayer`/`checkoutPlayer`/`changePlayerSkill`/automatic promotion-relegation ever touches `partnerId`, and `extractFixedPartnerTeams`, `BalancedRotationEngine.js`, simply skips force-pairing whenever one side of a mutual link isn't currently in the eligible matchmaking pool — held, checked out, or in the other Adaptive Skill Rotation division — and resumes force-pairing the moment both are back in the same pool again); this sprint makes the guarantee explicit and adds direct regression coverage for every named scenario. **Create Partner** / **Change Partner** / **Remove Partner** are the existing `setFixedPartner(state, idA, idB)` / `clearFixedPartner(state, playerId)` (`lib/queueManagement.js`) — the same always-visible "+ Partner" picker in the Waiting Players panel, unchanged.
  - ✅ Dynamic Court Count — organizers can now remove a court at any point during an active session, not just when it's already idle. New pure `requestRemoveCourt(state)` (`lib/queueManagement.js`) removes the last court immediately if it's idle ("open") right now — same numbering-stays-contiguous convention `addCourt` already relies on (`number: courts.length + 1`), so only ever the LAST court is ever removed, never a middle one that would require renumbering and invalidating `matchHistory`'s court references. If it isn't idle (a live match is on it), the removal is queued (`pendingCourtRemovals`, incremented) instead of blocked outright — never losing a live match. New `applyPendingCourtRemovals(state)` runs first thing inside `PickleballOpenPlay.jsx`'s `save()` (the same "reacts on every state change" precedent Smart Court Dispatch already established), completing the queued removal the instant that specific court's match ends and it resets to "open" — deliberately BEFORE Smart Court Dispatch runs in the same `save()` cycle, so a freshly-freed court a removal is waiting on gets removed instead of immediately re-filled with a new matchup. Smart Court Dispatch, the queue-depth cap (`maxUpcomingMatchups`), and voice announcements all read the courts array fresh every time, so none of them need separate wiring to "notice" a shorter courts array — it just works the next time any of them run. Never drops below 1 court. The Scorer toolbar's existing court +/- stepper's "−" button is always enabled now (previously disabled unless the last court was already idle); a small "N pending removal(s)" indicator appears beside it while one is queued.
  - ✅ Stop Queueing — a new Session Control button (`queueingStopped`, a plain state flag — not a Session Setting, since it's meant to be toggled quickly near the end of a session, not configured once upfront) in the Scorer toolbar, alongside a "Resume Queueing" to flip it back. While stopped: `save()` skips `refreshNextMatchups` entirely (existing `nextMatchups` pass through completely untouched — no new matchup is ever appended) and passes `autoFillCourts: false` to `dispatchAvailableCourts` regardless of the Auto-fill Courts session setting (no new automatic dispatch). Existing live matches and existing queued matchups are completely unaffected either way — a facilitator can still manually deploy anything already queued (`fillCourt`/`fillAllCourts` are untouched). "Generate remaining courts" and "Regenerate" — both of which explicitly build NEW matchups from scratch — are also no-ops (Generate falls back to just deploying whatever's already queued; Regenerate's button is disabled) while stopped, since both would otherwise contradict "no new matchups." A small banner ("Queueing is stopped — …") appears in the Scorer tab while active.
  - ✅ Dedicated Payment tab — all payment management moved out of the Scorer tab into its own top-level "Payment" tab, protected by the exact same Scorer PIN (`SCORER_PIN`) but its own independent authed state (unlocking Scorer doesn't automatically unlock Payment, and vice versa — same security-boundary precedent Scorer PIN auth's own "not persisted across a refresh" already set). New `PaymentView.jsx` lists every checked-in player alphabetically (regardless of queue status — waiting, upcoming, playing, held, or already checked out; payment is independent of match participation) with the exact same `PaymentBadge` (`UP`/`P-C`/`P-GC`, one-click Cash/GCash/Undo) the Scorer tab used to render inline — same component, same `setPlayerPayment` handler, byte-for-byte identical behavior, only its location moved — plus the same Total Players/Paid/Unpaid/Cash/GCash stats panel (`derivePaymentStats`, unchanged). Payment badges are now completely removed from the Scorer tab: `PlayerChip.jsx` (shared by court/matchup player rows) gained a `hidePayment` prop, threaded through `TeamRow`/`CourtCard`/`NextMatchupCard` and set wherever those are rendered inside Scorer (mirroring the existing `hideAvatar` precedent exactly) — `BoardView`/`TournamentCourtsView`, which never pass it, are completely unaffected. The Waiting Players panel's inline `PaymentBadge`/`onSetPayment` are removed outright (Scorer-only, no other caller). Session History's payment reporting (`computeSessionAnalyticsReport`'s Payment Summary/per-player Payment Detail) is completely untouched — it already reads straight from `state.players`, same as this new tab does.
...
- [x] **Standalone Double Elimination Tournaments** (2026-08-09) — real, fully playable Double Elimination, root-caused and rebuilt from a genuine production incident: Create Session's "Tournament Format" selector previously recorded a choice that had zero effect anywhere — `generateTournamentSchedule` (`PickleballOpenPlay.jsx`) unconditionally called `buildAndSaveRoundRobinTournament` regardless of the selected format, so a real event configured as "Double Elimination" silently ran as plain Round Robin (no elimination concept at all), which is why teams with 2 losses kept receiving matches. Fixed by actually branching on `state.tournamentFormat`.
  - **Standalone, no pool stage** — `buildAndSaveDoubleEliminationTournament` (`lib/tournament.js`) builds a Winners Bracket + Losers Bracket + Grand Final directly from this session's registered/checked-in players (`buildEntrants`, the same helper Round Robin uses), the moment the organizer clicks "Generate Double Elimination Bracket" on the Schedule tab — no Round Robin pool stage or qualification step first (a deliberate design choice, since Double Elimination and Round Robin are structurally different problems — see PROJECT.md's audit). Requires a power-of-two team count of at least 4. `tournament.pools` stays `[]`; entrants live in a new top-level `tournament.entrants` field instead (`lib/tournamentModel.js`), which `resolvePlayerIds` (`lib/tournament.js`) now falls back to for a standalone bracket's Player Database/rating hooks.
  - **Seeding reuses the existing mechanism** — `BracketSeeding.js`'s `random`/`rating` strategies (the two that don't need pool rank/pool-label data); Standard Cross-Pool/Snake are correctly not offered since they have nothing to seed from without pools. No new seeding logic was invented.
  - **Real elimination, structural (not a stored flag)** — completing the previously-half-built `DoubleEliminationEngine.js`: a Winners Bracket loser is now genuinely *seated* into their computed Losers Bracket destination (new `LosersBracketAdvancementService.seatWinnersBracketLoser`, replacing what used to be only a placeholder descriptor); a Losers Bracket match's winner advances (`LosersBracketAdvancementService.advanceWinner`, correctly handling both the "same round-pair size" 1:1 advance and the "crossing a pair boundary" halving advance the Losers Bracket's shape requires — verified against the standard 4/8/16-team shapes); its loser is eliminated by construction — no further match slot is ever generated for them anywhere, the same "elimination is implicit in bracket shape" precedent the existing single-elimination bracket already established. A pure, on-demand `DoubleEliminationEngine.getEliminatedParticipants(bracket)` (counts recorded losses ≥ 2 per participant across every completed match) drives the Bracket tab's "Eliminated" list — display-only, never consulted by advancement/seating logic itself.
  - **Grand Final + Reset, exact ruleset** — `DoubleEliminationEngine.populateGrandFinal`/`updateGrandFinal`: Game 1 seats the Winners Bracket champion (0 losses) against the Losers Bracket champion (1 loss) the moment both are known; the Winners Bracket champion winning Game 1 ends the tournament immediately (no reset); the Losers Bracket champion winning Game 1 creates a Grand Final Reset (Game 2, winner-takes-all, same two teams) — the winner of Game 2 is champion regardless of which side they were. `saveGrandFinalMatchStart`/`saveGrandFinalMatchResult`/`saveGrandFinalWalkover` (`lib/tournament.js`) are new, dedicated save wrappers (the Grand Final's `{game1, game2}` container shape isn't a plain `{rounds}` bracket, so it doesn't reuse `PlayoffEngine.startMatch`/`updateBracket` directly).
  - **Score correction is provably safe** — bracket matches reuse `PlayoffAdvancementService.validateAdvancement` (composed by both `WinnersBracketAdvancementService` and the new `LosersBracketAdvancementService`), which already rejects re-saving a result onto an already-completed match outright ("advancing it again would advance the same participant twice") — the same behavior single-elimination has always had. Duplicate advancement/seating is structurally impossible, not just handled.
  - **Court assignment fully wired** — `CourtAssignmentService.collectMatches`/`updateMatchIn` now walk `tournament.doubleEliminationBracket` (Winners Bracket, Losers Bracket, Grand Final Game 1/Game 2) the same way `tournament.bracket`/`tournament.consolationBracket` already were — previously, a Double Elimination match could never be assigned to a court at all. An unpopulated future bracket slot (including every eliminated team's dead end) is never offered as playable, by the same existing `teamA && teamB` filter every other bracket format already relies on.
  - **Persistence/refresh** — verified live: refreshing mid-tournament (after a Winners Bracket result, after a team drops into the Losers Bracket, after the Losers Final, after Grand Final Game 1, after a Grand Final Reset) preserves bracket state exactly, including which Losers Bracket slots are seeded and the Grand Final's `game2`/`resetTriggered` fields.
  - **UI** — Create Session's Tournament Format selector now actually determines what gets built; Single Elimination is disabled and marked "Coming Soon" (still unimplemented) rather than silently accepted. The Bracket tab renders Winners Bracket / Losers Bracket / Grand Final (and Reset, when triggered) as fully real, playable sections — no more "structure-only preview," locked cards, or placeholder-only loser-destination text — plus Champion/Runner-up and an "Eliminated" list. The Overview tab gained a Double-Elimination-native summary (Winners/Losers Bracket status, Grand Final status including "Reset triggered", champion) instead of showing misleading zeroed-out pool statistics. The exact same completed engine also makes the pre-existing post-Round-Robin "Double Elimination" playoff format (Tournament Settings → Bracket Format) fully real for the first time — its "foundation-only preview" warning is removed.
  - 108 new regression checks (`scripts/verify-double-elimination.mjs`): bracket generation/shape for 4/8/16 teams, real seating, Losers Bracket advancement/elimination, Grand Final + Reset (both branches), the known match-count formulas (2N-2 without a reset, 2N-1 with one — confirmed against the actual implementation, not assumed), score-correction rejection, and court assignment integration. Full regression suite remains green (627/627 total). Live-verified end to end in the browser: a real 4-team standalone tournament played through Winners Bracket → Losers Bracket → Grand Final → Reset → Champion, refreshed at multiple checkpoints, zero console errors — including catching and fixing a real bug found only by live testing (the Grand Final Reset's Walkover button was wired to a no-op).
...
- [x] Manual Seeding
- [x] Double Elimination Brackets — see the dedicated section below (Standalone Double Elimination). Real, complete, playable — not a foundation-only preview.
- [x] Double Elimination Foundation
- [x] Winners Bracket Progression
- [x] Losers Bracket Progression
- [x] Grand Final Reset
