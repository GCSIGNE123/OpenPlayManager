...
export const PUBLIC_FINAL_PREFIX = "opl-public-final-"; // compact PUBLIC final summary of an ended session, keyed by sessionCode — exact-key read by the public /live viewer (see lib/publicFinalSummary.js). Deliberately not under opl-session-* so no prefix scan ever downloads it.
...
export const SESSION_INACTIVITY_AGE_MS = 24 * 60 * 60 * 1000; // 24-Hour Inactivity Auto-Close — a still-"active" session whose lastActivityAt (see defaultState.lastActivityAt / PickleballOpenPlay.jsx's save()) is at least this old gets automatically ended, independently of SESSION_AUTO_END_AGE_MS above — see lib/sessionIndexModel.js's sweepAgedSessions and lib/openPlaySessionLifecycle.js. Deliberately a SEPARATE constant, not a replacement: a session can be young (well under 3 days) and still be closed for inactivity, and a session can be actively played right up to the 3-day ceiling and still get closed there regardless of how recent its last activity was.
