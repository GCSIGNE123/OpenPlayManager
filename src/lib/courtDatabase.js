...
//   location ('indoor' | 'outdoor'), hourlyRate (nullable number — stored
//   only, not used by anything yet, per spec), active (bool),
//   createdAt, updatedAt (ms epoch)
// }
export function emptyCourtRecord({ name, number, location = "outdoor", hourlyRate = null }) {
  const now = Date.now();
  return {
    id: uid(),
