 ...
 * A SCOPED dark palette, deliberately NOT added to :root above: :root is
 * the CONNECT.PH palette every Open Play screen already relies on, and
 * this app has always used one shared palette for everything. Rather than
 * touch that (which would restyle Open Play too), this ruleset only
 * defines its custom properties under .tournament-dark — CSS custom
 * properties cascade normally to every descendant, inline-styled or not,
 * so any tournament component wrapped in this class can reference
 * var(--t-panel) etc. and get the dark theme, while Open Play (never
 * wrapped in this class) is completely unaffected and keeps reading the
 * :root values exactly as before. Values are the reference design's own
 * direction, given as the source of truth for this feature.
 * ------------------------------------------------------------------- */
.tournament-dark {
  --t-bg: #061525;
  --t-panel: #0B1B2D;
...
export const styles = {
  ...
  // Marketing Landing Page — see PROJECT.md/FEATURES.md. The real desktop-
  // width root cause (found by tracing the full DOM ancestor chain, not by
  // guessing): every OTHER screen renders inside `styles.app`'s
  // `overflow: hidden` card-shell wrapper, which is exactly right for the
  // ~900px-wide "app in a card" look every actual session screen uses —
  // but it also clips ANY child that tries to visually escape via a
  // negative-margin full-bleed trick, no matter how correct that child's
  // own CSS is. `mktPage`'s calc(50% - 50vw) full-bleed margins were
  // already computing correctly (verified: -301px at a 1470px viewport,
  // exactly right) — they were just being clipped one level up, at this
  // wrapper, which is why the page still measured ~868px (#root's content
  // width) instead of the viewport. `appLanding` is the one screen that
  // needs to escape that shell: same typography/color, but no
  // overflow/rounded-corner clipping. Applied ONLY when screen==="landing"
  // (see PickleballOpenPlay.jsx) — every other screen keeps `app` exactly
  // as before, unchanged.
  appLanding: {
    fontFamily: "'Inter', sans-serif",
    background: "var(--chalk)",
  ...
  serveBadge: (active) => ({
    ...
    fontWeight: 700,
    letterSpacing: "0.03em",
    color: active ? "var(--chalk)" : "var(--color-text-faint)",
    background: active ? "var(--court)" : "var(--chalk)",
  ...
  courtCardsGrid: {
    display: "grid",
    // auto-fit (not auto-fill) — with fewer courts than would fill a row,
    // auto-fill reserves empty tracks sized off the container width,
    // squeezing the real cards into narrow leftover columns even on a wide
    // screen. auto-fit collapses those empty tracks so existing cards
    // stretch to use the freed space instead (Courts View Responsive Fix).
  ...
  // Open Play Messaging (Player <-> Session Organizer) — Pro's
  // conversation list + thread. Compact list rows (organizer needs to
  // scan many players at a glance), minimal-click reply.
  messagesLayout: {
    display: "flex",
    gap: 16,
    alignItems: "flex-start",
  ...
  conversationRow: (active) => ({
    ...
    padding: "10px 12px",
    borderRadius: 8,
    border: `1.5px solid ${active ? "var(--ball)" : "var(--line)"}`,
    background: "var(--color-surface)",
    cursor: "pointer",
    textAlign: "left",
  }),
  ...
  messageBubble: (mine) => ({
    ...
    borderRadius: 12,
    fontSize: 13.5,
    background: mine ? "var(--ball)" : "var(--color-surface-alt, var(--color-surface))",
    border: mine ? "none" : "1px solid var(--line)",
    color: mine ? "#1a1a1a" : "var(--ink)",
    wordBreak: "break-word",
  }),
  messageBubbleTime: {
    display: "block",
  ...
  // ---------------------------------------------------------------------
  // Marketing Landing Page — see PROJECT.md/FEATURES.md. All new, additive
  // style keys (prefixed `mkt`) so nothing here touches the pre-existing
  // landing*/adminLink keys AccessScreen.jsx/PlayerPortalScreen.jsx/the old
  // LandingScreen.jsx already share — this is a completely separate visual
  // system living alongside them, not a replacement of shared tokens.
  // `mktPage` deliberately breaks out of index.css's `#root { max-width:
  // 900px }` (the classic full-bleed trick: relative + left 50% + negative
  // margin-left of 50vw + width 100vw) so the marketing page alone can use
  // the full viewport width like a real SaaS site, without touching
  // index.css itself — every other screen keeps its existing 900px cap.
  // ---------------------------------------------------------------------
  // Full-bleed trick, the scrollbar-safe variant — see PROJECT.md/
  // FEATURES.md. `calc(50% - 50vw)` margins (not the naive `left:50%` +
  // `margin-left:-50vw` positioning trick) because `50%` here resolves
  // against this element's own containing block (already scrollbar-safe,
  // inside `#root`), while `50vw` is the ideal viewport width INCLUDING the
  // vertical scrollbar's gutter — the naive trick's `left:50%` offset uses
  // that same mismatched `vw` for its own 50%, so the two errors don't
  // cancel and the page ends up shifted/clipped by roughly half the
  // scrollbar's width. This formulation only ever uses `vw` in the margin
  // itself, so the mismatch actually cancels out, which is what fixed the
  // "cards clipped/partially outside the visible area" symptom.
  mktPage: {
    width: "100vw",
    maxWidth: "100vw",
  ...
  // Hero — dark navy band, full-bleed, two-column (copy | product mockup)
  // collapsing to one column on mobile via CSS grid auto-fit-style inline
  // breakpoint handled by JS (see LandingScreen.jsx's useIsNarrow).
  mktHero: {
    background: "linear-gradient(160deg, var(--court-dark) 0%, var(--court) 100%)",
    color: "var(--chalk)",
  ...
  mktBtnGhostOnDark: {
    ...
    background: "transparent",
    color: "var(--chalk)",
    border: "1.5px solid rgba(243,241,228,0.4)",
    borderRadius: 9,
    padding: "14px 22px",
    fontWeight: 700,
    fontSize: 15,
  ...
  // Product mockup — a static, styled representation of the real PickleKing
  // UI (court/status/queue vocabulary lifted straight from CourtCard.jsx/
  // ScorerView.jsx's own labels and status colors), presented inside a
  // browser-chrome frame. Deliberately NOT the live components themselves
  // (no session data exists on the marketing page) — purely presentational,
  // never wired to onClick/state, so it can't be mistaken for real
  // functionality.
  mktMockupFrame: {
    background: "var(--color-surface)",
    borderRadius: 14,
  ...
  // Generic section scaffolding, reused by every content section below.
  mktSection: {
    padding: "72px 24px",
  },
  mktSectionAlt: {
  ...
  // Feature grid (Adaptive Rotation, etc.)
  mktFeatureGrid: {
    display: "grid",
  ...
  // Courts grid (LIVE / LIVE / NEXT / WAITING mini cards)
  mktCourtsGrid: {
    display: "grid",
    gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))",
    gap: 14,
  ...
  mktCourtMiniBadge: (status) => ({
    ...
    borderRadius: 999,
    padding: "4px 12px",
    color: status === "live" ? "var(--color-surface)" : status === "next" ? "var(--color-primary-dark)" : "var(--color-text-muted)",
    background: status === "live" ? "var(--color-success)" : status === "next" ? "var(--ball)" : "var(--color-bg)",
    border: status === "waiting" ? "1px solid var(--line)" : "none",
  }),
  // Latecomer priority demo strip
  mktLatecomerDemo: {
    display: "flex",
    alignItems: "center",
    gap: 16,
  ...
  mktFooterBottom: {
    ...
    width: "calc(100% - 64px)",
    margin: "0 auto",
    borderTop: "1px solid rgba(243,241,228,0.15)",
    paddingTop: 18,
    fontSize: 12,
    color: "rgba(243,241,228,0.45)",
    display: "flex",
  ...
  // Organizer/Admin/Developer tools — internal utilities, deliberately
  // de-emphasized (collapsed by default, muted styling) rather than
  // removed, per explicit "do not delete" instruction. Lives at the very
  // bottom of the page, below the marketing footer.
  mktToolsWrap: {
    background: "var(--color-primary-dark)",
  ...
  mktToolsLink: {
    background: "rgba(243,241,228,0.05)",
    border: "1px solid rgba(243,241,228,0.12)",
    borderRadius: 8,
  ...
  // FEATURES.md and fontImport's `.tournament-dark` ruleset above for the
  // scoped CSS vars these all reference. Tournament Mode ONLY — applied by
  // PickleballOpenPlay.jsx/TournamentDashboardView/TournamentCourtsView
  // exclusively when state.sessionType === "tournament"; Open Play keeps
  // using the existing `header`/`nav`/`main`/etc. styles above, untouched.
  // Every existing prop/handler these wrap is unchanged — visual only.
  // -------------------------------------------------------------------
  tShell: {
    background: "var(--t-bg)",
    color: "var(--t-text)",
  ...
  tLivePill: (active) => ({
    ...
    alignItems: "center",
    gap: 5,
    background: active ? "rgba(22,199,132,0.15)" : "rgba(159,176,195,0.12)",
    color: active ? "var(--t-live)" : "var(--t-muted)",
    border: `1px solid ${active ? "var(--t-live)" : "var(--t-border)"}`,
    borderRadius: 999,
    padding: "5px 12px",
    fontSize: 11.5,
    fontWeight: 700,
  ...
  tNavBtn: (active) => ({
    ...
    color: active ? "#FFFFFF" : "var(--t-muted)",
    border: `1px solid ${active ? "var(--t-primary)" : "transparent"}`,
    borderRadius: 8,
    padding: "8px 14px",
    fontSize: 12.5,
    fontWeight: 700,
  ...
  tCourtListItem: (selected) => ({
    background: selected ? "var(--t-elevated)" : "var(--t-panel)",
    border: `1.5px solid ${selected ? "var(--t-primary)" : "var(--t-border)"}`,
    borderRadius: 10,
    padding: "12px 14px",
    cursor: "pointer",
    textAlign: "left",
  ...
  tCourtStatusPill: (status) => ({
    ...
    padding: "2px 7px",
    borderRadius: 999,
    background: status === "live" ? "rgba(22,199,132,0.15)" : status === "upNext" ? "rgba(22,119,255,0.15)" : "rgba(159,176,195,0.12)",
    color: status === "live" ? "var(--t-live)" : status === "upNext" ? "var(--t-primary)" : "var(--t-muted)",
  }),
  tCourtListMatchup: {
    fontSize: 12.5,
    color: "var(--t-text)",
  ...
  tTeamCard: (serving) => ({
    ...
    minWidth: 220,
    background: "var(--t-panel)",
    border: `1.5px solid ${serving ? "var(--t-live)" : "var(--t-border)"}`,
    borderRadius: 12,
    padding: 16,
  }),
  tTeamCardHead: {
  ...
  tServeStatePill: (active) => ({
    ...
    padding: "4px 10px",
    borderRadius: 999,
    background: active ? "rgba(22,199,132,0.18)" : "rgba(159,176,195,0.12)",
    color: active ? "var(--t-live)" : "var(--t-muted)",
  ...
  tToggleBtn: (active) => ({
    ...
    padding: "8px 10px",
    borderRadius: 7,
    border: `1.5px solid ${active ? "var(--t-primary)" : "var(--t-border)"}`,
    background: active ? "var(--t-primary)" : "var(--t-elevated)",
    color: active ? "#FFFFFF" : "var(--t-muted)",
    fontWeight: 700,
    fontSize: 12,
    cursor: "pointer",
  ...
  tRotationValue: (tone) => ({
    ...
    alignItems: "center",
    justifyContent: "center",
    background: tone === "live" ? "var(--t-live)" : "var(--t-primary)",
    color: "#FFFFFF",
    borderRadius: 7,
    padding: "7px 14px",
    fontSize: 12.5,
  ...
  tStatusDot: (status) => ({
    ...
    height: 8,
    borderRadius: "50%",
    background: status === "inProgress" ? "var(--t-live)" : status === "completed" ? "var(--t-primary)" : "var(--t-muted)",
    display: "inline-block",
  }),
  tEmptyCourt: {
    fontSize: 13,
  ...
  // -------------------------------------------------------------------
  // Tournament Manager visual redesign, Stage 2 — see PROJECT.md/
  // FEATURES.md. Propagates the Stage 1 dark language (styles above) to
  // every remaining Tournament tab (Overview/Participants/Schedule/
  // Qualification/Seeding/Bracket/Standings/Reports/Settings). Every one
  // of those components only ever renders for a tournament session (same
  // reasoning as TournamentDashboardView/TournamentCourtsView in Stage 1),
  // so they reference these unconditionally — no Open Play branch needed.
  // Visual only: no calculation, data model, or handler these wrap is
  ...
  tBtnDisabled: { opacity: 0.4, cursor: "not-allowed" },
  tSubBtn: {
    width: 18,
    height: 18,
    borderRadius: "50%",
  ...
  tMatchCompletedAccent: {
    borderColor: "var(--t-live)",
  ...
  tMatchStatusBadge: (status) => ({
    fontFamily: "'Space Mono', monospace",
    fontSize: 10,
    fontWeight: 700,
    letterSpacing: "0.04em",
    textTransform: "uppercase",
    color: status === "completed" ? "#FFFFFF" : status === "locked" ? "var(--t-muted)" : "var(--t-text)",
    background:
      status === "completed"
        ? "var(--t-primary)"
        : status === "inProgress"
          ...
          : status === "paused"
            ...
            : status === "ready"
              ...
              : "var(--t-elevated)",
    border: status === "pending" || status === "ready" ? "1.5px solid var(--t-border)" : status === "locked" ? "1.5px dashed var(--t-border)" : "none",
    borderRadius: 5,
    padding: "3px 7px",
    flexShrink: 0,
    display: "inline-block",
  ...
  tWinnerSelectBtn: (active) => ({
    flex: 1,
    padding: "8px 10px",
    borderRadius: 7,
    border: `1.5px solid ${active ? "var(--t-live)" : "var(--t-border)"}`,
    background: active ? "rgba(22,199,132,0.12)" : "var(--t-elevated)",
    color: active ? "var(--t-live)" : "var(--t-muted)",
    fontWeight: 700,
    fontSize: 12,
    cursor: "pointer",
  ...
  tQueueName: { flex: 1, fontWeight: 600, color: "var(--t-text)" },
  tCourtBadgeDark: {
    display: "inline-flex",
    alignItems: "center",
    gap: 5,
  ...
  tSkillTag: (skill) => ({
    ...
    fontSize: 9.5,
    fontWeight: 700,
    color: skill === "intermediate" ? "#FFFFFF" : "var(--t-bg)",
    background: skill === "intermediate" ? "var(--t-primary)" : "var(--t-live)",
    borderRadius: 4,
    padding: "2px 5px",
    flexShrink: 0,
    letterSpacing: "0.03em",
  ...
  tQualTag: (status) => {
    const s = status === true ? "qualified" : status === false ? "eliminated" : status;
    const isQualified = s === "qualified" || s === "wildCard" || s === "bestThirdPlace" || s === "manualOverride";
    const color = isQualified ? "var(--t-live)" : s === "pending" ? "var(--t-primary)" : "var(--t-muted)";
    return {
      fontFamily: "'Space Mono', monospace",
      fontSize: 10,
      fontWeight: 700,
      ...
      textTransform: "uppercase",
      color,
      background: isQualified ? "rgba(22,199,132,0.12)" : s === "pending" ? "rgba(22,119,255,0.12)" : "var(--t-elevated)",
      border: `1px solid ${isQualified ? "var(--t-live)" : s === "pending" ? "var(--t-primary)" : "var(--t-border)"}`,
      borderRadius: 5,
      padding: "3px 7px",
      display: "inline-block",
    };
  ...
  tTableRow: (rank) => ({
    borderBottom: "1px solid var(--t-border)",
    background: rank <= 3 ? "rgba(22,119,255,0.06)" : "transparent",
  }),
  tTableCell: {
    padding: "8px",
    fontFamily: "'Space Mono', monospace",
  ...
  tTableDiffCell: (diff) => ({
    ...
    textAlign: "center",
    whiteSpace: "nowrap",
    color: diff > 0 ? "var(--t-live)" : diff < 0 ? "var(--t-danger)" : "var(--t-muted)",
  }),
  tSeedTag: {
    fontFamily: "'Space Mono', monospace",
    fontSize: 10.5,
  ...
  tProgressFill: (percent) => ({
    height: "100%",
    width: `${percent}%`,
    background: percent >= 100 ? "var(--t-live)" : "var(--t-primary)",
    transition: "width 0.4s ease",
  }),
  tRoundCard: {
    background: "var(--t-panel)",
  ...
  tVsLine: {
    borderTop: "1.5px dashed var(--t-border)",
    margin: "2px 0",
  },
};
